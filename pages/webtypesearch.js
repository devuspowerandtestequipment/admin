...
import {fetchAttributes,fetchCategories,fetchSubCategories,fetchChildCategories,fetchBrands,fetchTaxes,fetchDynamicDatas} from '../store/actions'
    ...
    getDatasPagination(datas){
        ...
        .then(response=>{
            ...
            if(this.state.sidebar_attributes.length===0){
                this.setState({
                    // sidebar_attributes:Object.assign(response.data.main_attributes, response.data.other_attributes),  //<<<<=========isue is in this line
            ...
            <Grid.Column  mobile={16} tablet={16} computer={16}>
                ...
                <Grid.Column mobile={16} tablet={8} computer={4}>
                    <div style={{backgroundColor:'#fff', padding:'10px', margin:'16px'}}>
                        ...
                        {/* {Object.entries(this.state.result_other_attributes).map((entry)=>{
                            return(
                                <>
                                <p><FindDynamicData redux_state={'attributes'} find='_id' value={entry[0]} get='name' /></p>
                                
                                {entry[1].map((da)=>{
                                    return(
                                        <p>--<FindDynamicData redux_state={`attributes`} parentAttributeId={entry[0]} find='_id' value={da} get='name' /> </p>
                                    )
                                })}
                                <br/><br/>
                                </>
...
export default connect(mapStateToProps, {fetchAttributes,fetchCategories,fetchSubCategories,fetchChildCategories,fetchBrands,fetchTaxes,fetchDynamicDatas})(withRouter(webtypesearch))